export const Author = "Eryk Wiśniewski 2G"
export const Version = "0.2.6"

export const listaPrzedmiotow = [
    ['Przedmiot', 0],
    ["Pm2", 1],
    ["Pm3", 1.5],
    ["Pm4", 2],
    ["Informatyka", 0.5],
    ["Fizyka", 1.5],
    ["Matematyka", 1],
    ["Polski", 1],
    ["Angielski", 0.5],
    ["Hiszpański", 1],
    ["Geografia", 1],
    ["Biologia", 0.5],
    ["Chemia", 1],
    ["Podstawy Przedsiębiorczości", 1],
    ["Historia", 1],
    ["Religia", 0.5],
    ["WF", 0.5],
    ["Godzina Wychowawcza", 1],
    ["Pm5", 2],
    ["Pm6", 2]

];

export const listaNauczycieli = [
    ["Nazwisko", "Imię", listaPrzedmiotow[0], 0],
    ["Bartosińska", "Grażyna", listaPrzedmiotow[1], 1],
    ["Słomkowska", "Monika", listaPrzedmiotow[2], 1],
    ["Słomkowska", "Monika", listaPrzedmiotow[17], 1],
    ["Rudzki", "Dariusz", listaPrzedmiotow[3], 2],
    ["Rudzki", "Dariusz", listaPrzedmiotow[18], 2],
    ["Rudzki", "Dariusz", listaPrzedmiotow[19], 2],
    ["Rychlik", "Jerzy", listaPrzedmiotow[4], 1],
    ["Staszewska", "Alina", listaPrzedmiotow[5], 1],
    ["Wróbel", "Danuta", listaPrzedmiotow[6], 1],
    ["Gaszczyńska", "Katarzyna", listaPrzedmiotow[7], 1],
    ["Domańska", "Beata", listaPrzedmiotow[8], 1],
    ["Klimczuk", "Małgorzata", listaPrzedmiotow[9], 1],
    ["Matysiak", "Agnieszka", listaPrzedmiotow[10], 1],
    ["Młynarczyk", "Małgorzata", listaPrzedmiotow[11], 1],
    ["Marszałek", "Ryszard", listaPrzedmiotow[12], 1],
    ["Kwaśnik", "Stanisław", listaPrzedmiotow[13], 1],
    ["Januszewska", "Maria", listaPrzedmiotow[14], 1],
    ["Kołak", "Irmina", listaPrzedmiotow[15], 1],
    ["Markowski", "Karol", listaPrzedmiotow[16], 1]
]
export const randomevent = [
    ['Co Będzie', 0],
    ["Normalna", 1],
    ["Kartkówka", 2],
    ["Sprawdzian", 3],
    ["Odwołana/Zastępstow", 0]
]


const assetsCon = true;
export default assetsCon
